...
module.exports.run = async (bot: any, message: Message, args: string[]) => {
    ...
    let source: any = await fetch(sourceAPI);
    ...
    if (results[0].data.ext_urls != undefined) {
        ...
        shortUrl.shorten(args.join('%20'), function (shortReq: string, err: Error) {
            reqURL = shortReq
        });
        shortUrl.shorten(results[0].data.ext_urls[0], function (short1URL: string, err: Error) {
    ...
    if (results[1] != undefined && results[1].data.ext_urls != undefined) {
        ...
        shortUrl.shorten(results[1].data.ext_urls[0], function (short2URL: string, err: Error) {
    ...
    if (results[2] != undefined && results[2].data.ext_urls != undefined) {
        ...
        shortUrl.shorten(results[2].data.ext_urls[0], function (short3URL: string, err: Error) {
